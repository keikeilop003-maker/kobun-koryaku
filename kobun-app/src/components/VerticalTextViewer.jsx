import { splitParallelViewLines, splitViewLineEntries, splitModernForSourceLines, editorInitialParallelText } from '../utils/lessonViewText';
...
function NotesEditor({ section, initialText, onSave }) {
  ...
  const save = async () => {
    ...
    setMessage('');
    try {
      if (!onSave) throw new Error('保存処理が利用できません。ページを再読み込みしてください。');
